<?php
    ...
    if (file_exists($modelFilePath)) {
        if (unlink($modelFilePath)) {
            ...
            $modelBaseName = pathinfo($model, PATHINFO_FILENAME);
        
            // Delete it also from the models table (use the basename)
            $sql = "DELETE FROM models WHERE name_of_model = ?";
            $stmt = $mysqli->prepare($sql);
            $stmt->bind_param('s', $modelBaseName);
            $stmt->execute();
            $stmt->close();
        
            echo json_encode(["status" => "success", "message" => "Model deleted successfully", "model" => $modelBaseName]);
        } else {
