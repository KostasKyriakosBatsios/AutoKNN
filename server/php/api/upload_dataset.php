<?php
    ...
    $query = "SELECT id, email, allowPublic FROM users WHERE token = ?";
    $stmt = $mysqli->prepare($query);
    $stmt->bind_param("s", $token);
    $stmt->execute();
    $stmt->bind_result($userId, $email, $allowPublicFromDb);
    $stmt->fetch();
    $stmt->close();
    ...
    $uploadDir = '../../python/';
    if ($folder === 'public') {
        if ($allowPublicFromDb == 0) {
            http_response_code(403);
            echo json_encode(["status" => "danger", "message" => "You are not allowed to upload to the public folder."]);
            exit;
        }
        $uploadPath = $uploadDir . 'public/datasets';
    } else {
        $uploadPath = $uploadDir . 'private/' . $userHash . '/datasets';
    }
